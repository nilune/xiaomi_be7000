...
3. Также скачиваем последнее ядро `xray` (<https://github.com/xtls/xray-core/releases>) с расширением `linux-arm64-v8a`. Распаковываем и переименованием `xray` в `v2ray`.
4. На внешнем накопителе в папке `System` создать подпапку `v2ray` (тут будут храниться все необходимое для сервисов). В эту папку копируем всё из папки `data`:
