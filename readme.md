...
3. Ставим первый сервис - `portrainer` - позволяет легко управлять контейнерами (вместо дефолтного, так как он не удобен). Сам сервис будет доступен в UI по адресу <http://${ROUTER_ADDRESS}:9000>. Заходим по ssh на роутер и запускаем на нем (поменяй путь согласно твоему имени флешки):
...
- [ ] Описание как делать бекап (xmir, ui, отдельные файлы)
